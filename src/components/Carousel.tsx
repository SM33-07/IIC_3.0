...
function CarouselItem({ item, index, itemWidth, round, trackItemOffset, x, transition }: CarouselItemProps) {
  ...
  return (
    <motion.div
      ...
      className={`relative shrink-0 flex flex-col overflow-hidden cursor-grab active:cursor-grabbing border border-white/10 rounded-[16px] group`}
      style={{
        width: itemWidth,
        height: '100%',
        rotateY: rotateY,
    ...
    >
      ...
      <div className="relative z-10 p-6 w-full h-full flex flex-col justify-between pointer-events-none">
        ...
        <div>
          <div className="mb-1 font-black text-lg text-white drop-shadow-md">{item.title}</div>
          <p className="text-sm text-gray-200 drop-shadow-sm">{item.description}</p>
        </div>
      </div>
    </motion.div>
  );
...
export default function Carousel({
  ...
  return (
    <div
      ref={containerRef}
      className={`relative overflow-hidden p-4 bg-black/10 backdrop-blur-sm ${
        round ? 'rounded-full border border-white' : 'rounded-[24px] border border-white/5'
      }`}
      style={{
        width: `${width}px`,
